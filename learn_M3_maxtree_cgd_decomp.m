function learn_M3_maxtree_cgd_decomp(params)
...
xlocksize = eDom(end,2);
d_x = zeros(xlocksize,1);
...
if params.profiling
    ...
    mu = reshape( mu, 4, size( E, 1)*size(Kx_ts, 1));
...
while and(primal_ub - obj > params.epsilon*obj,iter < maxiter)
    ...
    for i = 1:length(Chunk)
        ...
        x = Chunk(i);
        x_vars = (x-1)*xlocksize + consistent_vars;
        loss_x = loss(:,x);
        mu_x = mu(:,x);
        te_x = t_E(:,x);
...
%		  mu2 = mu;
%		  mu2(:,x) = reshape(mu_x,4*size(E,1),1);
%       obj11 = sum(sum(loss.*mu2));
%        obj12 = - 1/2*sum( sum( reshape(mu2,4,size(E,1)*m) .* compute_Kmu( reshape( mu2,4,size(E,1)*m),Kx_tr,1:m,1:m) ) );
%        obj1 = obj11 + obj12;
%        fprintf(['\n\tToinen tulostus : Iteratiivinen obj = %f\t kerralla laskettu obj = %f ' ...
%        '%f %f\n'], obj, obj1, obj11, obj12);
  ...
  %          round(tm-start_time),x,iter,obj,primal_ub-obj,sum(xi));
    ...
    while max_chunk < m
    	...
    	Kmu(:,chunk) = reshape(compute_Kmu(mu,Kx_tr,chunk),4*size(E,1),length(chunk));
    end
...
while iter < maxiter
    ...
    mu_1 = max_tree_direction_x(gradient,mu_x,x,C,0);

    d_x = mu_1 - mu_x;

    if sum(mu_1) == C*size(E,1)
